...
def getChamberBoxes(chambersmask):
    ...
    if chambersmask.dtype == bool:
        ...
        chambersmask = cv2.threshold(chambersmask, 0.5, 1, cv2.THRESH_BINARY)[1].astype(np.uint8)
...
def getSinglecells(seg):
    ...
    for c, contour in enumerate(contours):
        singlecellseg[c] = cv2.drawContours(
            np.zeros((seg.shape[0], seg.shape[1]), dtype=np.uint8),
            [contour],
            0,
            color=1,
            thickness=-1,
...
def getTrackingScores(inputs, outputs, method="histogram"):
    ...
    if method == "overlap":
        ...
        for o in range(outputs.shape[0]):
            mother_pred = cv2.threshold(outputs[o, :, :, 0], 0.5, 1, cv2.THRESH_BINARY)[1].astype(
                np.uint8
            )
            daughter_pred = cv2.threshold(outputs[o, :, :, 1], 0.5, 1, cv2.THRESH_BINARY)[1].astype(
                np.uint8
            )
...
def getOverlap(output, target):
    ...
    """
    return cv2.sumElems(cv2.bitwise_and(output, target))[0] / cv2.sumElems(target)[0]
...
def getAttributions(scores):
    ...
    for i in range(scores.shape[1]):  # Go over cells in the *new* frame
        if np.sum(scores[:, i, :] > 0) == 1:  # If only one old cell was attributed to the new cell
            ...
            if (
                np.sum(scores == np.max(scores[:, i, :])) == 1
            ):  # One cell has a higher score than the others
                attrib[:, i, :] = scores[:, i, :] == np.max(scores[:, i, :])
            elif np.any(
                scores[:, i, 1] == np.max(scores[:, i, :])
            ):  # At least one of the highest scoring old cells would be the mother of this new "daughter"
                attrib[np.argmax(scores[:, i, 1]), i, 1] = (
                    True  # keep only the first one (which is also the one higher in the image)
                )
            else:  # If only mother-to-mother couplings are in conflict
                attrib[np.argmax(scores[:, i, 0]), i, 0] = (
                    True  # keep only the first one (which is also the one higher in the image)
                )
    for o in range(scores.shape[0]):  # Go over cells in the *old* frame
        if (
            np.sum(attrib[o, :, 0] > 0) > 1
        ):  # If one old cell gets attributed to more than one new cell (mother-mother coupling)
            tokeep = np.argmax(attrib[o, :, 0])
            attrib[o, :, 0] = False
            attrib[o, tokeep, 0] = True  # keep only the first one
        if (
            np.sum(attrib[o, :, 1] > 0) > 1
        ):  # If one old cell gets attributed to more than one new cell (mother-daughter coupling)
            tokeep = np.argmax(attrib[o, :, 1])
            attrib[o, :, 1] = False
            attrib[o, tokeep, 1] = True  # keep only the first one
        if (
            np.any(attrib[o, :, 0])
            and np.any(attrib[o, :, 1])
            and np.argmax(attrib[o, :, 0]) > np.argmax(attrib[o, :, 1])
        ):  # If somehow the mother is lower than the daughter
        ...
        if np.any(attrib[o, :, 1]) and not np.any(
            attrib[o, :, 0]
        ):  # If a cell is attributed a daughter but somehow no mother-mother coupling
            attrib[o, np.argmax(attrib[o, :, 1]), 0] = True
            attrib[o, np.argmax(attrib[o, :, 1]), 1] = False
    return attrib
...
def label_seg(seg, cellnumbers=None):
    ...
    contours.sort(key=lambda elem: np.max(elem[:, 0, 1]))  # Sorting along Y
    label = np.zeros(
        seg.shape, dtype=np.uint16
    )  # Because we use uint16, we can not have >65535 cells in one movie
    for c, contour in enumerate(contours):
...
def updatelineage(newframe, stack, framenb=0, lineage=None, attrib=None):
    ...
    if lineage == None:  # Not initialized yet
        ...
        for c in cells[1:]:  # Avoiding cell==0 (background)
            lineage.append(
                createcell(framenb + 1)
            )  # We're using framenb +1 to be compatible with old Matlab code
        ...
        if not np.any(
            label_old > 0
        ):  # This can happen if the chamber is empty and suddenly cells appear (can be a segmentation error, or a focus problem)
            contours = cv2.findContours(
                newframe.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )[
                0
            ]  # In this case just label
        ...
        # If previous stack frame has cells:
            ...
            for i in range(attrib.shape[1]):  # Go through new cells
                if np.any(attrib[:, i, 0]):  # If cell is tracked from an old cell: (mother-mother)
                    cellnum = int(
                        stack[framenb - 1][label_old == (np.nonzero(attrib[:, i, 0])[0] + 1)[0]][0]
                    )  # retrieving "mother" cell number from old stack frame
                    lineage[cellnum - 1]["framenbs"].append(
                        float(framenb + 1)
                    )  # Update frame number list
                    if len(lineage[cellnum - 1]["daughters"]) < len(
                        lineage[cellnum - 1]["framenbs"]
                    ):  # If framenbs hasn't been updated (in case somehow daughter goes through before mother)
                        lineage[cellnum - 1]["daughters"].append(0.0)
                    cellnumbers.append(cellnum)
                elif np.any(
                    attrib[:, i, 1]
                ):  # Cell is tracked as a daughter cell (mother-daughter)
                    mothernb = int(
                        stack[framenb - 1][label_old == (np.nonzero(attrib[:, i, 1])[0] + 1)[0]][0]
                    )  # retrieving cell number from stack
                    # Create new cell, as daughter of mother:
                    lineage.append(createcell(framenb + 1, mothernb))
                    cellnumbers.append(len(lineage))
                    # Update 'daughters' key of mother cell:
                    if len(lineage[mothernb - 1]["daughters"]) < len(
                        lineage[mothernb - 1]["framenbs"]
                    ):  # This situation should never happen actually
...
def createcell(framenb, mothernb=0):
    ...
    """
    return dict(
        framenbs=[float(framenb)],  # For compatibility with old version of the pipeline
        mothernb=mothernb,
        daughters=[0.0],
    )
...
def legacysave(res_file, xpfile, delta_res, chamberboxes, rot_corr, driftvalues, moviedimensions):
    """
    ...
    if delta_res is not None:
        savemat(
            res_file,
            {
                ...
                "proc": {
                    ...
                    "chambers": np.array(
                        [
                            [c["xtl"], c["ytl"], c["xbr"] - c["xtl"], c["ybr"] - c["ytl"]]
                            for c in chamberboxes
                        ],
                        dtype=np.float64,
                    ),
        ...
        savemat(
            res_file,
            {
                ...
                "tiffile": xpfile,
                "moviedimensions": moviedimensions,
                "proc": {"rotation": rot_corr, "chambers": [], "XYdrift": []},
            },
        )
...
def results_movie(xp, pos, res, chamberboxes, rot_corr=0, driftvalues=None, display=False):
    """
    ...
    # Get trans frames:
    transframes = xp.getframes(
        positions=pos, channels=0, rescale=(0, 1), rotate=rot_corr
    )  # Load all trans frames, with rotation correction
            ...
            for chamber, chamberbox in enumerate(chamberboxes):
                ...
                for c, cell in enumerate(cells):
                    ...
                    frame = cv2.drawContours(
                        ...
                        color=colors[cell],
                        thickness=1,
                        offset=(chamberbox["xtl"], chamberbox["ytl"]),
                    )
    ...
    if get_contours:
        ...
        for c, cell in enumerate(cells):
            ...
            cnt, _ = cv2.findContours(
                (labels == cell + 1).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
            contours += cnt
...
def vidwrite(images, filename, crf=20, verbose=1):
    ...
    process = (
        ffmpeg.input(
            "pipe:", format="rawvideo", pix_fmt="rgb24", s="{}x{}".format(width, height), r=7
        )
        ...
        if use_bioformats:
            ...
            if prototype is None:
                imgfiles = [
                    ...
                    if os.path.splitext(x)[1].lower() in (".tif", ".tiff")
                ]
            ...
            if "p" in self.fileorder:
                self.positions = 0
                while os.path.exists(self.getfilenamefromprototype(self.positions, 0, 0)):
                    self.positions += 1
            else:
                self.positions = 1
            ...
            if "c" in self.fileorder:
                self.channels = 0
                while os.path.exists(self.getfilenamefromprototype(0, self.channels, 0)):
                    self.channels += 1
            else:
                self.channels = 1
            ...
            if "t" in self.fileorder:
                self.timepoints = 0
                while os.path.exists(self.getfilenamefromprototype(0, 0, self.timepoints)):
                    self.timepoints += 1
            else:
    ...
    def getframes(
        ...
        if channels is None:  # No channel specified: load all
            ...
            if len(channels) == 0:  # Empty list of channels: Return None
                ...
                [type(c) is str for c in channels]
        ...
        if resize is None:
            ...
            output = np.empty(
                [len(positions), len(frames), len(channels), resize[0], resize[1]], dtype=dt
            )
            ...
            for c, cha in enumerate(channels):
                for f, fra in enumerate(frames):
                    ...
                    if self.use_bioformats:
                        ...
                        frame = cv2.imread(
                            self.getfilenamefromprototype(pos, cha, fra), cv2.IMREAD_ANYDEPTH
                        )
                    elif self.filetype == "tif":
                        frame = (
                            self.filehandle.series[pos].pages[fra * self.channels + cha].asarray()
                        )
                    # Optionally resize and rescale:
                    if rotate is not None:
                        M = cv2.getRotationMatrix2D((self.x / 2, self.y / 2), rotate, 1)
                        frame = cv2.warpAffine(
                            frame, M, (self.x, self.y), borderMode=cv2.BORDER_REPLICATE
                        )
