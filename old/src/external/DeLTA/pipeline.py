...
def process_xp(
    ...
    for x, experiment in enumerate(xpfile):
        ...
        if verbose >= 1:
            print(
                "\n%s ----- Processing experiment %d/%d: %s"
                % (
                    time.strftime("%e %b %Y - %T", time.localtime()),
                    x + 1,
                    len(xpfile),
        ...
        for pos in positions_torun:
            ...
            delta_output = process_position(xp, pos, verbose=verbose)

            # Save results to disk:
            utils.legacysave(
                os.path.join(resfolder, ("Position%06d.mat" % (pos + 1))),
                experiment,
                *delta_output,
            ...
            if write_movie:
                ...
                movie = utils.results_movie(xp, pos, *delta_output, display=False)
                utils.vidwrite(
                    movie * 255,
    ...
    if verbose >= 1:
        print(
            "\n\n%s ----- %d/%d experiments processed"
            % (time.strftime("%e %b %Y - %T", time.localtime()), x + 1, len(xpfile))
        )


def process_position(xp, pos, rotation_correction=True, verbose=1):
    """
    ...
    if chamberboxes is None:
        if verbose:
            print("\b\b\b - No chambers detected! Aborting...")
    ...
    ##### Tracking:
    res = trackinglineagefeatures(transframes, fluoframes, seg, seg_inputs, chamberboxes)
...
def preprocessing(xp, pos, rotation_correction=True):
    ...
    firstframe = xp.getframes(
        positions=pos, channels=0, frames=0, rescale=(0, 1)
    )  # Get first frame for rotation estimation (We assume first channel is trans)
    # Rotation correction:
    ...
    )  # Now load all trans frames, with rotation correction
    firstframe = np.expand_dims(
        np.expand_dims(cv2.resize(transframes[0], (512, 512)), axis=0), axis=3
    ...
    chambermask = cv2.resize(
        np.squeeze(chambermask), transframes.shape[3:0:-1]
    )  # scaling back to original size
    chambermask = postprocess(
        chambermask, min_size=cfg.min_chamber_area
    )  # Binarization, cleaning and area filtering
    ...
    driftcorbox = dict(
        ...
        ybr=(
            ...
            else max(chamberboxes, key=lambda elem: elem["ytl"])["ytl"]
        ),
    )  # Box to match template
    transframes, driftvalues = utils.driftcorr(
        transframes, template=drifttemplate, box=driftcorbox
    )  # Run drift corr
    # Load up fluoresence images, apply drift correction and rotation:
    fluoframes = xp.getframes(
        positions=pos,
        channels=[x for x in range(1, xp.channels)],
    ...
    if fluoframes is not None:
        ...
        for f in range(fluoframes.shape[1]):  # Go over all channels:
            fluoframes[:, f, :, :], _ = utils.driftcorr(
                fluoframes[:, f, :, :], drift=driftvalues
...
def segmentation(transframes, chamberboxes):
    ...
    for m, chamberbox in enumerate(chamberboxes):
        for i in range(transframes.shape[0]):
            seg_inputs.append(
                cv2.resize(
                    utils.rangescale(utils.cropbox(transframes[i], chamberbox), (0, 1)), (32, 256)
                )
            )
...
def trackinglineagefeatures(transframes, fluoframes, seg, seg_inputs, chamberboxes, verbose=1):
    """
    ...
    for m, chamberbox in enumerate(chamberboxes):
        ...
        #### Tracking Unet:
        track, track_inputs, frame_numbers = tracking(seg, seg_inputs, m, transframes.shape[0])

        ##### Compile lineage and label stack:
        lin, label_stack = lineage(seg, track, track_inputs, frame_numbers, m, transframes.shape[0])

        ##### Extract features:
        lin, label_stack_resized = features(
            lin, label_stack, fluoframes, chamberbox, transframes.shape[0]
        )

        # Append to res dictionary:
        res.append(
            dict(lineage=lin, labelsstack=label_stack, labelsstack_resized=label_stack_resized)
        )
...
def tracking(seg, seg_inputs, chamber_number, timepoints):
    ...
    for i in range(1, timepoints):
        singleseg = utils.getSinglecells(
            seg[chamber_number * timepoints + i - 1]
        )  # Get single cell masks in the 'old' frame
            ...
            track_inputs.append(
                np.stack(
                    (
                        ...
                        seg_inputs[
                            chamber_number * timepoints + i - 1, :, :, 0
                        ],  # Previous trans image
                        seg[chamber_number * timepoints + i],
                    ),  # Mask of all current cells
                    axis=-1,
                )
            )
...
def lineage(seg, track, track_inputs, frame_numbers, chamber_number, timepoints):
    ...
    lin, label_stack = utils.updatelineage(
        seg[chamber_number * timepoints], label_stack
    )  # Initialize lineage and label stack on first frame
    for i in range(1, timepoints):
        frame_idxs = [x for x, fn in enumerate(frame_numbers) if fn == i]
        if frame_idxs:
            scores = utils.getTrackingScores(
                track_inputs[frame_idxs[0], :, :, 3], track[frame_idxs]
            )
            attrib = utils.getAttributions(scores)
        else:
            attrib = []
        lin, label_stack = utils.updatelineage(
            seg[chamber_number * timepoints + i], label_stack, framenb=i, lineage=lin, attrib=attrib
        )  # Because we use uint16, we can only track up to 65535 cells per chamber
...
def features(lin, label_stack, fluoframes, chamberbox, timepoints):
    ...
    label_stack_resized = np.empty(
        [timepoints, chamberbox["ybr"] - chamberbox["ytl"], chamberbox["xbr"] - chamberbox["xtl"]],
        dtype=np.uint16,
    )
    for i in range(timepoints):  # Go over frames
        # Resize from original stack:
        label_stack_resized[i] = cv2.resize(
            label_stack[i], label_stack_resized.shape[2:0:-1], interpolation=cv2.INTER_NEAREST
        )  # The stack needs to be resized to the actual chamber box size
        ...
        for c, cellnb in enumerate(cells):  # Run through cells in frame
            ...
            lin[cellnb]["pixels"].append(
                np.ravel_multi_index(cellpixels, label_stack_resized.shape[1:]).astype(np.float32)
            )  # Using floats for compatibility with previous version of the pipeline
            if fluoframes is not None:
                for f in range(fluoframes.shape[1]):  # Go over all channels:
                    lin[cellnb]["fluo" + str(f + 1)].append(
