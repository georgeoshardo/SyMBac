...
class ColonySimulation:
    ...
    def run_cellmodeller_sim(self, num_sim):
        for n in range(num_sim):
            ...
            self.simulation = Simulator(
                self.cellmodeller_model,
                dt=0.025,
                clPlatformNum=0,
                clDeviceNum=0,
                saveOutput=True,
                pickleSteps=1,
                outputDirName=f"{self.save_dir}/{str(self.n_simulations)}",
                is_gui=True,
            )
    ...
    def get_cellmodeller_properties(self, cellmodeller_pickle):
        ...
        for cell in cellmodeller_pickle["cellStates"].values():
            angle = (
                np.rad2deg(
                    np.arctan2(cell.ends[1][1] - cell.ends[0][1], cell.ends[1][0] - cell.ends[0][0])
                )
                + 180
                + 90
            )
    ...
    def get_simulation_pickles(self):
        ...
        for dir in self.get_simulation_dirs():
            pickles.append(natsorted(glob(f"{dir}/*.pickle")))
    ...
    def get_scene_size(self, cellmodeller_properties):
        ...
        scene_shape = (
            np.ceil(max_y + abs(min_y) + max_cell_length * 2).astype(int),
    ...
    def draw_scene(
        ...
        # sample OPL cell
        if as_3D:
            OPL_cell = raster_cell(
                cellmodeller_properties[0][0], cellmodeller_properties[0][1], separation=0
            )
        ...
        if random_distribution == "uniform":
            density_modifiers = [
                np.random.uniform(*distribution_args) for _ in range(len(cellmodeller_properties))
            ]
        if random_distribution == "normal":
            density_modifiers = [
                np.random.normal(*distribution_args) for _ in range(len(cellmodeller_properties))
            ]
            density_modifiers = [
                x if x > 0 else np.mean(density_modifiers) for x in density_modifiers
            ]
        for c in range(len(cellmodeller_properties)):
            ...
            OPL_cell = raster_cell(
                cellmodeller_properties[c][0], cellmodeller_properties[c][1], separation=0
            )
            if as_3D:
                OPL_cell_3D = convert_to_3D(OPL_cell)
                OPL_cell_3D = np.array(
                    [
                        rotate(
                            x,
                            -cellmodeller_properties[c][2],
                            resize=True,
                            clip=False,
                            preserve_range=True,
                        )
                        for x in OPL_cell_3D
                    ]
                )
            rotated_OPL_cell = rotate(
                OPL_cell,
                -(cellmodeller_properties[c][2]),
                resize=True,
                clip=False,
            ...
            offset_y = (
                rotated_OPL_cell.shape[0]
                - space[y - cell_y : y + cell_y, x - cell_x : x + cell_x].shape[0]
            )
            offset_x = (
                rotated_OPL_cell.shape[1]
            ...
            if as_3D:
                ...
                if FL:
                    FL_cell_3D = np.array(
                        [OPL_to_FL(x, density=density * density_modifiers[c]) for x in OPL_cell_3D]
                    )
                    fluorescence_space_3D[
                        :, y - cell_y : y + cell_y + offset_y, x - cell_x : x + cell_x + offset_x
                    ] += FL_cell_3D
            ...
            mask[y - cell_y : y + cell_y + offset_y, x - cell_x : x + cell_x + offset_x] += (
                (rotated_OPL_cell) > 1
            ) * mask_counter
            mask_counter += 1
        ...
        if crop:
            ...
            if FL:
                fluorescence_space = crop_image(
                    fluorescence_space, (start_row, stop_row), (start_col, stop_col), pad=crop_pad
                )
                if as_3D:
                    fluorescence_space_3D = crop_image(
                        fluorescence_space_3D,
                        (start_row, stop_row),
                        (start_col, stop_col),
                        pad=crop_pad,
                    ...
                    space_3D = crop_image(
                        space_3D, (start_row, stop_row), (start_col, stop_col), pad=crop_pad
                    )
        ...
        if save and savename:
            ...
            if as_3D:
                tifffile.imwrite(
                    f"{self.projections_dir}/{savename}.tif",
                    space_3D.astype(np.uint8),
                    compression="zlib",
                    compressionargs={"level": 8},
                )
                if FL:
                    tifffile.imwrite(
                        f"{self.fluorescent_projections_dir}/{savename}.tif",
                        fluorescence_space_3D.astype(np.uint8),
                        compression="zlib",
                        compressionargs={"level": 8},
                    )
    ...
    def draw_OPL_from_idx(
        self,
        idx,
        FL=False,
        density=1,
        random_distribution="uniform",
        distribution_args=(0.99, 1.01),
    ):
        cellmodeller_properties = self.get_cellmodeller_properties(
            self.pickle_opener(self.pickles_flat[idx])
        )
        return self.draw_scene(
            cellmodeller_properties,
            False,
            False,
            False,
    ...
    def draw_simulation_OPL(
        ...
        all_cellmodeller_properties = [
            self.get_cellmodeller_properties(self.pickle_opener(_)) for _ in self.pickles_flat
        ]

        n_files = len(glob("data/scenes/*.png"))
        zero_pads = np.ceil(np.log10(len(self.pickles_flat))).astype(int) + 2
        Parallel(n_jobs=n_jobs)(
            delayed(self.draw_scene)(
                _,
                True,
                str(i + 1 + n_files).zfill(zero_pads),
                False,
                FL,
                density,
                random_distribution,
            ...
            for i, _ in tqdm(enumerate(all_cellmodeller_properties), desc="Scene Draw:")
        )
