...
class ColonyRenderer:
    ...
    def perlin_generator(self, shape, scale=5, octaves=10, persistence=1.9, lacunarity=1.8):
        ...
        y, x = np.round(shape[0] / self.resize_amount).astype(int), np.round(
            shape[1] / self.resize_amount
        ).astype(int)
    ...
    def random_perlin_generator(self, shape):
        return self.perlin_generator(
            shape,
            np.random.uniform(1, 7),
            np.random.choice([10, 11, 12, 13]),
    ...
    def render_scene(self, idx):
        ...
        if "3d" in self.PSF.mode.lower():
            ...
            if self.force_2D:
                ...
                convolved = convolve_rescale(
                    scene, kernel, 1 / self.resize_amount, rescale_int=False
                )
            else:
                convolved = np.array(
                    [
                        convolve_rescale(
                            ...
                            1 / self.resize_amount,
                            rescale_int=False,
                        )
                        for scene_slice, PSF_slice in zip(scene, kernel)
                    ]
                )
        else:
        ...
        if "phase" in self.PSF.mode.lower():
            ...
            convolved += gaussian_filter(
                np.rot90(bg) / np.random.uniform(500, 1500), np.random.uniform(1, 3), mode="reflect"
            )
    ...
    def generate_random_samples_ray(
        self, n, roll_prob, savedir, GPUs=(0,), n_jobs=1, gpu_fraction=1, batch_size=20
    ):
        n_GPUs = len(GPUs)
        # if n_GPUs > 1:
        #    n_jobs = n_GPUs
                ...
                def run_batch(j, i):
                    if j > n:
                        ...
                        if np.random.rand() < roll_prob:
                            n_axis_to_roll, amount = random.choice(
                                [
                                    (0, int(sample.shape[0] / 2)),
                                    (1, int(sample.shape[1] / 2)),
                                    ([0, 1], (int(sample.shape[0] / 2), int(sample.shape[1] / 2))),
                                ]
                            )
                        ...
                        if "3d" in self.PSF.mode.lower():
                            tifffile.imwrite(
                                f"{savedir}/synth_imgs/{str(i).zfill(zero_pads)}.tif",
                                sample,
                                compression="zlib",
                            ...
                            Image.fromarray(sample).save(
                                f"{savedir}/synth_imgs/{str(i).zfill(zero_pads)}.png"
                            )
                        Image.fromarray(rescaled_mask).save(
                            f"{savedir}/masks/{str(i).zfill(zero_pads)}.png"
                        )
        ...
        batched_idxs = batched(
            islice(enumerate(cycle(range(len(self.OPL_dirs)))), 0, n), batch_size
        )
        ...
        ray.get(
            [run_on_GPU.remote(batch, zero_pads, gpu_id, n_jobs) for batch, gpu_id in batched_zip]
        )
