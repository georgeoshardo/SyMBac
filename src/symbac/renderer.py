...
from symbac.pySHINE import cart2pol, lumMatch, sfMatch
...
if importlib.util.find_spec("cupy") is None:
    ...
    njobs = -1
    warnings.warn(
        "Could not load CuPy for SyMBac, are you using a GPU? Defaulting to CPU convolution."
    )
...
class Renderer:
    ...
    def __init__(self, simulation, PSF, real_image, camera=None, additional_real_images=None):
        ...
        temp_expanded_scene, temp_expanded_scene_no_cells, temp_expanded_mask = (
            self.generate_PC_OPL(
        ...
        self.y_border_expansion_coefficient = (
            np.ceil(real_image.shape[0] / temp_expanded_scene.shape[0]) * simulation.resize_amount
        ) + 3
        self.x_border_expansion_coefficient = (
            np.ceil(real_image.shape[1] / temp_expanded_scene.shape[1]) * simulation.resize_amount
        ) + 3
        ...
        convolved = convolve_rescale(
            temp_expanded_scene, temp_kernel, 1 / simulation.resize_amount, rescale_int=True
        )
        print(simulation.resize_amount, convolved.shape)
        self.real_resize, self.expanded_resized = make_images_same_shape(
            real_image, convolved, rescale_int=False
        )
    ...
    def generate_test_comparison(
        ...
        noise_var=0.001,
        defocus=3.0,
        halo_top_intensity=1,
        halo_bottom_intensity=1,
        halo_start=0,
        halo_end=1,
        ...
        ### Halo simulation
        def halo_line_profile(
            length, halo_top_intensity, halo_bottom_intensity, halo_start, halo_end
        ):
        ...
        if self.PSF.mode.lower() == "3d fluo":  # Full 3D PSF model
            ...
            def gen_3D_coords_from_2D(centreline, thickness):
                return np.where(test_cells == thickness) + (
                    generate_deviation_from_CL(centreline, thickness),
                )
            ...
            psf = psfm.make_psf(
                volume_shape[2], radius * 2, dxy=scale, dz=scale, pz=0, ni=n, wvl=λ, NA=NA
            )
            ...
            convolved = convolve_rescale(
                expanded_scene, kernel, 1 / self.simulation.resize_amount, rescale_int=True
            )  #
        # real_resize, OPL_scene = make_images_same_shape(self.real_image, expanded_scene, rescale_int=False)
        real_resize, convolved = make_images_same_shape(
            self.real_image, convolved, rescale_int=True
        )
        real_resize = (
            real_resize / real_resize.max()
        )  # Because we do not want to rescale the convolution in the above line
        ...
        if match_fourier and not match_histogram:
            ...
            matched = lumMatch(
                [real_resize, matched], None, [np.mean(real_resize), np.std(real_resize)]
            )[1]
        else:
            matched = convolved
        ...
        if self.camera:  # Camera noise simulation
            baseline, sensitivity, dark_noise = (
                self.camera.baseline,
                self.camera.sensitivity,
                self.camera.dark_noise,
            )
            ...
            noisy_img = random_noise(
                rescale_intensity(noisy_img), mode="gaussian", mean=0, var=noise_var, clip=False
            )
        ...
        expanded_mask_resized = rescale(
            expanded_mask,
            1 / self.simulation.resize_amount,
            anti_aliasing=False,
            preserve_range=True,
            order=0,
        )
        if len(np.unique(expanded_mask_resized)) > 2:
            _, expanded_mask_resized_reshaped = make_images_same_shape(
        ...
        expanded_media_mask = rescale(
            (expanded_scene_no_cells == device_multiplier)
            ^ (expanded_scene - expanded_scene_no_cells).astype(bool),
            1 / self.simulation.resize_amount,
            anti_aliasing=False,
        )
        _, expanded_media_mask = make_images_same_shape(
            self.real_image, expanded_media_mask, rescale_int=False
        )
        just_media = expanded_media_mask * noisy_img

        expanded_cell_pseudo_mask = (expanded_scene - expanded_scene_no_cells).astype(bool)
        expanded_cell_pseudo_mask = rescale(
            expanded_cell_pseudo_mask, 1 / self.simulation.resize_amount, anti_aliasing=False
        )

        _, expanded_cell_pseudo_mask = make_images_same_shape(
            self.real_image, expanded_cell_pseudo_mask, rescale_int=False
        )
        just_cells = expanded_cell_pseudo_mask * noisy_img

        expanded_device_mask = expanded_scene_no_cells

        expanded_device_mask = rescale(
            expanded_device_mask, 1 / self.simulation.resize_amount, anti_aliasing=False
        )

        _, expanded_device_mask = make_images_same_shape(
            self.real_image, expanded_device_mask, rescale_int=False
        )
        ...
        simulated_means = np.array(
            [
                just_media[np.where(just_media)].mean(),
                just_cells[np.where(just_cells)].mean(),
                just_device[np.where(just_device)].mean(),
            ]
        )
        simulated_vars = np.array(
            [
                just_media[np.where(just_media)].var(),
                just_cells[np.where(just_cells)].var(),
        ...
        if debug_plot:
            ...
            return (
                noisy_img,
                expanded_mask_resized_reshaped.astype(int),
                superres_mask.astype(int),
                expanded_scene,
            )
        ...
        def get_OPL_image(
            ...
            if "fluo" in self.PSF.mode.lower():
                ...
                rr, cc = draw.rectangle(
                    start=segment_1_top_left, end=segment_1_bottom_right, shape=test_scene.shape
                )
                test_scene[rr, cc] = 1 * media_multiplier

                rr, cc = draw.rectangle(
                    start=segment_2_top_left, end=segment_2_bottom_right, shape=test_scene.shape
                )
                test_scene[rr, cc] = 1 * media_multiplier

                circ_midpoint_y = (segment_1_top_left[1] + segment_2_bottom_right[1]) / 2
                radius = (
                    segment_1_top_left[1]
                    - self.simulation.offset
                    - (segment_2_bottom_right[1] - self.simulation.offset)
                ) / 2
                circ_midpoint_x = (self.simulation.offset) + radius

                rr, cc = draw.rectangle(
                    start=segment_2_top_left,
                    end=(circ_midpoint_x, segment_1_top_left[1]),
                ...
                rr, cc = draw.disk(
                    center=(circ_midpoint_x, circ_midpoint_y), radius=radius, shape=test_scene.shape
                )
            ...
            test_scene = test_scene[
                segment_2_top_left[0] : segment_1_bottom_right[0],
            ...
            expanded_scene_no_cells = (
                np.zeros(
                    (
                        int(no_cells.shape[0] * y_border_expansion_coefficient),
                        int(no_cells.shape[1] * x_border_expansion_coefficient),
                    )
                )
                + media_multiplier
            )
            expanded_scene_no_cells[
                expanded_scene_no_cells.shape[0] - no_cells.shape[0] :,
                int(expanded_scene_no_cells.shape[1] / 2 - int(test_scene.shape[1] / 2)) : int(
                    expanded_scene_no_cells.shape[1] / 2 - int(test_scene.shape[1] / 2)
                )
                + no_cells.shape[1],
            ] = no_cells
            if "fluo" in self.PSF.mode.lower():
                expanded_scene = np.zeros(
                    (
                        int(test_scene.shape[0] * y_border_expansion_coefficient),
                        int(test_scene.shape[1] * x_border_expansion_coefficient),
                    )
                )
                expanded_scene[
                    expanded_scene.shape[0] - test_scene.shape[0] :,
                    int(expanded_scene.shape[1] / 2 - int(test_scene.shape[1] / 2)) : int(
                        expanded_scene.shape[1] / 2 - int(test_scene.shape[1] / 2)
                    )
                    + test_scene.shape[1],
                ] = test_scene
            else:
            ...
            expanded_mask[
                expanded_mask.shape[0] - test_scene.shape[0] :,
                int(expanded_mask.shape[1] / 2 - int(test_scene.shape[1] / 2)) : int(
                    expanded_mask.shape[1] / 2 - int(test_scene.shape[1] / 2)
                )
                + test_scene.shape[1],
            ] = mask_resized
    ...
    def optimise_synth_image(self, manual_update):
        ...
        self.image_params = (
            self.real_media_mean,
            self.real_cell_mean,
            self.real_device_mean,
            self.real_means,
            self.real_media_var,
            self.real_cell_var,
            self.real_device_var,
            self.real_vars,
        )
    ...
    def generate_training_data(
        ...
        mask_dtype=np.uint8,
        dry_run=False,
        render_sample_parameters=None,
        randomise_fourier_match=False,
        prefix=None,
    ):
        """
        ...
        def generate_samples(
            ...
            syn_image.save(
                "{}/convolutions/{}synth_{}.png".format(save_dir, prefix, str(z).zfill(5))
            )
            OPL_scene_image = Image.fromarray(skimage.img_as_uint(rescale_intensity(OPL_scene)))
            OPL_scene_image.save(
                "{}/OPL_scenes/{}synth_{}.png".format(save_dir, prefix, str(z).zfill(5))
            )
            ...
            if (cell_multiplier == 0) or (cell_multiplier == 0.0):
                ...
                superres_mask.save(
                    "{}/superres_masks/{}synth_{}.png".format(save_dir, str(z).zfill(5))
                )
            else:
                mask = Image.fromarray(mask.astype(mask_dtype))
                mask.save("{}/masks/{}synth_{}.png".format(save_dir, prefix, str(z).zfill(5)))
                ...
                superres_mask = Image.fromarray(superres_mask.astype(mask_dtype))
                superres_mask.save(
                    "{}/superres_masks/{}synth_{}.png".format(save_dir, prefix, str(z).zfill(5))
                )
            ...
            if in_series:
                ...
                media_multipliers = np.repeat(
                    [
                        ...
                        * self.params.kwargs["media_multiplier"]
                ...
                cell_multipliers = np.repeat(
                    [
                        np.random.uniform(1 - sample_amount, 1 + sample_amount)
                        * self.params.kwargs["cell_multiplier"]
                        for _ in range(n_series_to_sim)
                    ],
                    series_len,
                )
                device_multipliers = np.repeat(
                    [
                        np.random.uniform(1 - sample_amount, 1 + sample_amount)
                        * self.params.kwargs["device_multiplier"]
                        for _ in range(n_series_to_sim)
                    ],
                    series_len,
                )
                sigmas = np.repeat(
                    [
                        np.random.uniform(1 - sample_amount, 1 + sample_amount)
                        * self.params.kwargs["sigma"]
                        for _ in range(n_series_to_sim)
                    ],
                    series_len,
                )
                scene_nos = (
                    np.arange(burn_in, self.simulation.sim_length).tolist() * n_series_to_sim
                )
                ...
                scene_nos = np.random.randint(
                    low=burn_in, high=self.simulation.sim_length - 2, size=n_samples
                )
        ...
        Parallel(n_jobs=n_jobs, backend="threading")(
            ...
            for z, media_multiplier, cell_multiplier, device_multiplier, sigma, scene_no, match_histogram, match_noise, match_fourier in tqdm(
                zip(
                    range(render_sample_parameters["n_samples"]),
