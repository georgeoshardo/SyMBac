...
def crop_image(img, rows, cols, pad):
    ...
    if len(img.shape) == 3:
        return np.pad(
            img[:, start_row:stop_row, start_col:stop_col], ((0, 0), (pad, pad), (pad, pad))
        )
    else:
...
def raster_cell(length, width, separation, pinching=True, FL=False):
    ...
    for c in range(len(sphere_Rs)):
        ...
        new_cell[c, int(W / 2) - sphere_Rs[c] : int(W / 2) + sphere_Rs[c]] = np.concatenate(
            (I_cyl, I_cyl[::-1])
        )
        new_cell[L - c - 1, int(W / 2) - sphere_Rs[c] : int(W / 2) + sphere_Rs[c]] = np.concatenate(
            (I_cyl, I_cyl[::-1])
        )
    ...
    if separation > 2 and pinching:
        ...
        for c in range(int((S + 1) / 2)):
            ...
            new_cell[int((L - S) / 2) + c + 1, int(W / 2) - R__ : int(W / 2) + R__] = (
                np.concatenate((I_cyl_, I_cyl_[::-1]))
            )
            new_cell[-int((L - S) / 2) - c - 1, int(W / 2) - R__ : int(W / 2) + R__] = (
                np.concatenate((I_cyl_, I_cyl_[::-1]))
            )
...
def OPL_to_FL(cell, density):
    ...
    choices = np.searchsorted(
        np.cumsum(weights), np.random.rand(n_molecules)
    )  # workaround for np.random.choice from
    ...
    for properties in cell_properties:
        ...
        for i in range(OPL_cell.shape[0]):
            for j in range(OPL_cell.shape[1]):
                noise[i][j] = pnoise2(
                    (i - OPL_cell.shape[0] + texture_y_coordinate) / 2 / scale,
        ...
        if do_transformation:
            ...
            OPL_cell_2[
                :,
                midpoint
                - int(OPL_cell.shape[1] / 2) : midpoint
                - int(OPL_cell.shape[1] / 2)
                + OPL_cell.shape[1],
            ] = OPL_cell
        ...
        rotated_OPL_cell = rotate(
            OPL_cell, -angle, resize=True, clip=False, preserve_range=True, center=(x, y)
        )
        ...
        assert (
            y > cell_y
        ), f"Cell has {y - cell_y} negative pixels in y coordinate, try increasing your offset".format(
            y - cell_y
        )
        assert (
            x > cell_x
        ), f"Cell has {x - cell_x} negative pixels in x coordinate, try increasing your offset"
        space[
        ...
        def get_mask(label_masks):
            ...
            if label_masks:
                space_masks_label[
                    y - cell_y : y + cell_y + offset_y, x - cell_x : x + cell_x + offset_x
                ] += (rotated_OPL_cell > 0) * sim_mask_label
                # colour_label[0] += 1
...
def place_cell(length, width, angle, position, space):
    ...
    cell_y, cell_x = (np.array(rotated_OPL_cell.shape) / 2).astype(int)
    offset_y = (
        rotated_OPL_cell.shape[0] - space[y - cell_y : y + cell_y, x - cell_x : x + cell_x].shape[0]
    )
    offset_x = (
        rotated_OPL_cell.shape[1] - space[y - cell_y : y + cell_y, x - cell_x : x + cell_x].shape[1]
    )
    space[
        y - cell_y + 100 : y + cell_y + offset_y + 100,
        x - cell_x + 100 : x + cell_x + offset_x + 100,
    ] += rotated_OPL_cell
...
def transform_func(amp_modif, freq_modif, phase_modif):
    def perm_transform_func(x, amp_mult, freq_mult, phase_mult):
        return (
            amp_mult
            * amp_modif
            * np.cos((x / (freq_mult * freq_modif) - phase_mult * phase_modif) * np.pi)
        ).astype(int)
...
def make_images_same_shape(real_image, synthetic_image, rescale_int=True):
    ...
    assert (
        real_image.shape[0] < synthetic_image.shape[0]
    ), "Real image has a higher diemsion on axis 0, increase y_border_expansion_coefficient"
    assert (
        real_image.shape[1] < synthetic_image.shape[1]
    ), "Real image has a higher diemsion on axis 1, increase x_border_expansion_coefficient"
        ...
        if synthetic_image.shape[1] % 2 == 0:
            if y_diff > 0:
                synthetic_image = synthetic_image[
                    y_diff:, remove_from_left - 1 : -remove_from_right
                ]
            else:
...
def draw_simulation_OPL(simulation, do_transformation=True, label_masks=True):
    ...
    cell_timeseries_properties = Parallel(n_jobs=-1)(
        ...
        for a in tqdm(
            simulation.cell_timeseries, desc="Extracting cell properties from the simulation"
        )
    )
    ...
    scenes = Parallel(n_jobs=-1)(
        delayed(draw_scene)(cell_properties, do_transformation, space_size, offset, label_masks)
