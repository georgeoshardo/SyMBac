    ...
    def calculate_PSF(self):
        if "phase contrast" in self.mode.lower():
            with warnings.catch_warnings():
                ...
                self.kernel = self.get_phase_contrast_kernel(
                    R=self.R,
                    W=self.W,
                    radius=self.radius,
                    scale=self.scale,
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                self.kernel = self.get_fluorescence_kernel(
                    radius=self.radius,
                    scale=self.scale,
                    NA=self.NA,
                    n=self.n,
            ...
            raise NameError(
                "Incorrect mode, currently supported: phase contrast, simple fluo, 3d flup"
            )
