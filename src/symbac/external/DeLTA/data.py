...
def illumination_voodoo(image, num_control_points=5):
    ...
    newimage = np.multiply(
        image,
        np.reshape(
            np.tile(np.reshape(curve, curve.shape + (1,)), (1, image.shape[1])), image.shape
        ),
    )
...
def trainGenerator_track(
    ...
    while True:
        ...
        for _ in range(batch_size):
            ...
            mother = readreshape(
                os.path.join(mother_path, os.path.basename(filename)),
                target_size=target_size,
                binarize=True,
                order=0,
                rangescale=False,
            )
            daughter = readreshape(
                os.path.join(daughter_path, os.path.basename(filename)),
                target_size=target_size,
                binarize=True,
                order=0,
                rangescale=False,
...
def predictCompilefromseg_track(img_path, seg_path, files_list=[], target_size=(256, 32)):
    """
    ...
    for index, item in enumerate(files_list):
        ...
        if fra > 1:
            ...
            previmg = readreshape(
                os.path.join(img_path, prevframename), target_size=target_size, order=1
            )
...
def estimateClassweights(gene, num_samples=30):
    ...
    class_weights = [
        (x / num_samples) ** -1 if x != 0 else 0 for x in class_counts
    ]  # Normalize by nb of samples and invert to get weigths, unless x == 0 to avoid Infinite weights or errors
