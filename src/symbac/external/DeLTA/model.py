...
from tensorflow.keras.layers import Concatenate, Conv2D, Dropout, Input, MaxPooling2D, UpSampling2D
...
def unet_seg(input_size=(256, 32, 1)):
    ...
    model.compile(
        optimizer=Adam(lr=1e-4), loss=pixelwise_weighted_binary_crossentropy, metrics=[unstack_acc]
    )
...
def unet_track(input_size=(256, 32, 4), class_weights=(1.0, 1.0, 1.0)):
    """
