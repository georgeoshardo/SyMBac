...
**Update** There is currently a big update coming which will feature lineage tracking ability, full texture generation within cells, spatial fluorescence (membrane, nucleoid, ribosome, etc), and more advanced image simulation models, and BENDY cells! It will also (hopefully) bring about full modularity of SyMBac so you can plug in individual parts of the pipeline to your own workflows. Additionally, preliminary support for using SyMBac entirely from Napari is coming. You can follow the progress in the `full_rework` branch. 
